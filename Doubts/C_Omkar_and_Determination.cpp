...
void getprefix(vector<vector<int>>& prefixsum){
    for(int i = prefixsum.size()-2; i >= 0; i--){
        prefixsum[i][0] += prefixsum[i+1][0];
    }
    for(int i = 1; i < prefixsum[0].size(); i++){
        prefixsum[prefixsum.size()-1][i] += prefixsum[prefixsum.size()-1][i-1];
    }
    for(int i = prefixsum.size()-2; i >= 0; i--){
...
void solve(){
    ...
    for(int i = 0; i<n; i++){
        for(int j = 0; j<m; j++){
            if((i > 0 && j > 0) && (arr[i-1][j] == 'X' && arr[i][j-1] == 'X')) prefixsum[i][j] = 1;
        }
    }
    getprefix(prefixsum);
    cin>>q;
    for(int i = 0; i<q; i++){
        int start, end;
        cin>>start>>end;
        if(prefixsum[0][start-1] == prefixsum[0][end-1]){
            cout<<"YES"<<endl;
        }
        else cout<<"NO"<<endl;
    }
